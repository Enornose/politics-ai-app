...
app.get('/auth/callback', async (req, res) => {
  ...
  try {
    ...
    if (data.access_token) {
      ...
      return res.status(500).send('Failed: ' + JSON.stringify(data));
    }
    res.redirect('/');
  } catch (err) {
...
app.get('/api/products', async (req, res) => {
  ...
  try {
    const url = 'https://' + SHOPIFY_STORE + '/admin/api/2024-01/products.json?limit=50&fields=id,title,images,body_html&status=any';
  ...
  try {
    ...
    if (imageUrl) {
      ...
      messageContent = [
        ...
        { type: 'text', text: 'Product title: ' + title + '\nAnalyze the image and fill in each field accurately.' }
      ];
    } else {
